...
import { AuthContext, setAuthToken } from '@package/api';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Restore the in-memory bearer token from the cookie-backed session so
        // Authorization-header auth keeps working after a reload / PWA relaunch
        // (sessionStorage is empty then, and mobile Safari may not send the
        // cross-site auth cookie on later fetches).
        if (meData.accessToken) setAuthToken(meData.accessToken);
