...
import type { DutchRegion, Partner, TrainerSpecialization } from '@package/api';
...
const PartnerCard = memo(function PartnerCard({ partner }: PartnerCardProps) {
  ...
  return (
    <StyledCard $variant="neutral" $overflow="hidden" style={{ padding: 0 }}>
      ...
      <FlexRow $justify="space-between" $align="center" $pt="sm" $pl="md" $pr="md" $pb="md">
        ...
        {partner.price_from === null && <div />}

        <Button $variant="secondary" $size="small" $borderRadius="full" onClick={handleViewClick}>
      ...
      if (regionFilter) {
        result = result.filter((partner) =>
          partner.regions && partner.regions.length > 0
            ? partner.regions.includes(regionFilter as DutchRegion)
            : partner.region === regionFilter
        );
      }
