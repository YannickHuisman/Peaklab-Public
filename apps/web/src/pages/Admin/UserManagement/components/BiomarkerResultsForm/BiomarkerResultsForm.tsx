...
export function BiomarkerResultsForm({
  testId,
  panelId,
  defaultShowAll = false,
  onUpdate,
}: BiomarkerResultsFormProps) {
  ...
  const [showPanelOnly, setShowPanelOnly] = useState(!defaultShowAll);

  const fetchPanelBiomarkers = useCallback(async () => {
    try {
      const response = await authenticatedFetch('/api/biomarkers/admin/all');
  ...
  }, [testId, fetchPanelBiomarkers, fetchExistingResults]);
  ...
  const groupedBiomarkers = panelBiomarkers
    .filter((b) => b.kind !== 'ratio' && b.kind !== 'calculated')
    .filter((b) => !showPanelOnly || b.panels.some((p) => p.panel_id === panelId))
    .reduce(
      (acc, biomarker) => {
    ...
    <StyledCard $variant="small" $showBorder $noShadow $mt="md">
      <StyledResultsHeader>
        ...
        <ToggleGroup
          ...
          value={showPanelOnly ? 'panel' : 'all'}
          onChange={(v) => setShowPanelOnly(v === 'panel')}
        />
                ...
                {biomarkers.map((biomarker) => {
                  ...
                  return (
                    <BiomarkerCard
                      ...
                      isInPanel={biomarker.panels.some((p) => p.panel_id === panelId)}
