...
import type { BiomarkerCategory, BiomarkerKind, BiomarkerWithConfig } from '@package/api';

import { StyledCheckboxWrapper, StyledFormGrid, StyledLabel, StyledSelect } from '../styles';
import { DependencySelector } from './DependencySelector';
import { PanelSelector } from './PanelSelector';
import { RangeRow } from './RangeRow';
import type { BiomarkerFormData, DependencyFormItem } from './types';

const KIND_OPTIONS: Array<{ value: BiomarkerKind; label: string }> = [
  { value: 'direct', label: 'Direct (gemeten in lab)' },
  { value: 'ratio', label: 'Ratio (verhouding van twee biomarkers)' },
  { value: 'calculated', label: 'Berekende waarde (formule met meerdere bronnen)' },
];
...
export function BasicInfoTab({
  ...
  const isDerived = values.kind === 'ratio' || values.kind === 'calculated';
  ...
  return (
    <StyledFormGrid>
      ...
      <FlexColumn $gap="xxs">
        ...
        <StyledSelect
          value={values.kind}
          onChange={(e) => setFieldValue('kind', e.target.value as BiomarkerKind)}
        >
      ...
      {isDerived && (
        <DependencySelector
          ...
          onChange={(deps: DependencyFormItem[]) => setFieldValue('dependencies', deps)}
          availableBiomarkers={availableBiomarkers}
          excludeId={currentBiomarkerId}
          formula={values.formula}
          onFormulaChange={(f) => setFieldValue('formula', f)}
        />
      )}
