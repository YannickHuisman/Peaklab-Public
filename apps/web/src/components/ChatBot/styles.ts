...
export const StyledMobilePanel = styled.div<{ $side: 'left' | 'right'; $active: boolean }>`
  display: contents;
  ...
  @media (max-width: 767px) {
    ...
    transform: ${({ $side, $active }) =>
      $active ? 'translateX(0)' : $side === 'left' ? 'translateX(-100%)' : 'translateX(100%)'};
...
export const StyledConversationActions = styled.div`
  ...
  gap: 2px;
  flex-shrink: 0;
  /* Visible by default on touch devices. A hover-reveal there swallows the
     first tap (iOS applies :hover instead of firing the click), which forced a
     double-tap to open a chat. Only hide-until-hover on real pointer devices. */
  opacity: 1;
