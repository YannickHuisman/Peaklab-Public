...
const NAVIGATE_MAP: Record<string, string> = {
  ...
  deep_research_completed: '/deep-research',
};
  ...
  return (
    <StyledNotificationWrapper ref={dropdownRef}>
      <Button $variant="ghost" onClick={handleToggle}>
        <Icons.Bell color={iconColor} />
        {unreadCount > 0 && (
          <StyledNotificationBadge>{unreadCount > 9 ? '9+' : unreadCount}</StyledNotificationBadge>
        )}
      </Button>
          ...
          {notifications.length === 0 ? (
            ...
            <FlexColumn $gap="xs">
              {notifications.map((notification) => (
                ...
                >
                  {!notification.is_read && <StyledNotificationDot />}
                  <FlexColumn $gap="xxs">
