...
import systemConfigRouter from './routes/systemConfigRouter';
import subscriptionRouter from './routes/subscriptionRouter/subscriptionRouter';
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      // Allow local network IPs in development (e.g. mobile on same WiFi)
      const isLocalNetwork = /^https?:\/\/192\.168\.\d+\.\d+/.test(origin ?? '');
